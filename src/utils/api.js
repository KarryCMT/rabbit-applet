...
import { getRabbitConsolePrefix } from '@/utils/config';
        ...
        Object.keys(apis).forEach((apiKey) => {
          if (this.apis[`${reqDefault.name}.${apiKey}`]) {
            if (process.env.RUN_ENV !== 'plugin') {
              console.error(
                `${getRabbitConsolePrefix('utils-api')}名称为${
                  reqDefault.name
                }.${apiKey}的接口已经存在，请勿重复注册`
              );
            }
            return;
          }
...
class RequestClass {
  ...
  request(string, options) {
    ...
    if (!apiString) {
      console.error(
        `${getRabbitConsolePrefix(
          'utils-api'
        )}未找到名称为${string}的api,请确认已在api中注册`
      );
      return;
    }
